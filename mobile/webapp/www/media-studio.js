...
//
// Adaptations par rapport au VPS (pas de serveur Node local sur mobile,
// contrairement à local-client/) :
//  - POST /api/media/creative-direction (LLM structuré) -> réutilise
...
//    Cloud Functions que la vidéo IA générique).
...
(function () {
  ...
  function getLicenseKey() { return localStorage.getItem('cyrus_license_key') || ''; }
  function aiHeaders() { return { 'Content-Type': 'application/json', 'x-license-key': getLicenseKey(), 'x-device-id': getDeviceId() }; }
  ...
  let mediaCurrentCreation = null; // { img, promptEnriched, width, height, ratio, deliverable, usedFallback }
  let mediaVariantSeq = 0;
  let mediaCreativeDirection = null; // { ...directive, forConcept }
  ...
  // ---------- Creative Director IA (adapté : generateTextFallback + JSON) ----------
  async function mediaFetchCreativeDirection() {
    ...
    try {
      const prompt = 'Tu es un directeur créatif publicitaire. Pour la demande suivante, réponds UNIQUEMENT avec un objet JSON strict (aucun texte autour, aucun markdown), avec exactement ces clés : '
      ...
      let directive;
      try {
        const jsonMatch = String(data.text || '').match(/\{[\s\S]*\}/);
        directive = JSON.parse(jsonMatch ? jsonMatch[0] : data.text);
      } catch (e) {
        throw new Error('Réponse IA non exploitable (format inattendu) — renseignez les champs manuellement.');
      }
      ...
      mediaCreativeDirectorFeedback('✅ Direction créative générée (' + (data.provider || 'IA') + ') — champs pré-remplis, modifiez-les librement.', false);
    } catch (err) {
  ...
  document.getElementById('media-deliverable-toggle').addEventListener('click', (e) => {
    ...
    const isVideo = btn.dataset.deliverable === 'VIDEO_SHORT';
    document.getElementById('media-video-fields').style.display = isVideo ? 'block' : 'none';
    document.getElementById('media-export-video-btn').style.display = isVideo ? 'inline-block' : 'none';
    document.getElementById('media-export-ai-video-btn').style.display = isVideo ? 'inline-block' : 'none';
    document.getElementById('media-ai-video-provider').style.display = isVideo ? 'block' : 'none';
  });

  // Pré-sélection par mots-clés (même parseur qu'avant, aucune IA générative)
  ...
  function mediaIntentParse(text) {
    ...
    return { ratio: formatMatch ? formatMatch.ratio : null, deliverable: deliverableMatch ? deliverableMatch.deliverable : 'FLYER_AFFICHE' };
  }
  document.getElementById('media-concept').addEventListener('change', () => {
    ...
    if (ratio && !mediaState.ratio) { const b = document.querySelector('#media-ratio-toggle .toggle-btn[data-ratio="' + ratio + '"]'); if (b) b.click(); }
    if (deliverable && !mediaState.deliverable) { const b = document.querySelector('#media-deliverable-toggle .toggle-btn[data-deliverable="' + deliverable + '"]'); if (b) b.click(); }
  });

  // ---------- Traducteur/enrichisseur de prompt (identique au VPS) ----------
  ...
  function mediaBuildEnrichedPrompt(conceptRaw, deliverable, sector, extraNote) {
    const hasFreshDirection = mediaCreativeDirection && mediaCreativeDirection.forConcept === conceptRaw.trim() && mediaCreativeDirection.imagePromptEnglish;
    const base = hasFreshDirection ? mediaCreativeDirection.imagePromptEnglish
  ...
  function mediaLoadImageUrl(url, onSettle) {
    ...
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true; img.onload = null; img.onerror = null; img.src = '';
      const err = new Error('Le service de génération met trop de temps à répondre — réessayez.'); err.kind = 'timeout';
      onSettle(err, null);
    }, MEDIA_FETCH_TIMEOUT_MS);
    img.onload = () => { if (settled) return; settled = true; clearTimeout(timer); onSettle(null, img); };
    img.onerror = () => {
      if (settled) return; settled = true; clearTimeout(timer);
      const err = new Error('Échec de génération du fond IA — réessayez.'); err.kind = 'error';
      onSettle(err, null);
    };
    img.src = url;
  }
  ...
  async function mediaFetchBackgroundImage(promptEnriched, w, h) {
    ...
    for (let attempt = 0; attempt <= backoffsMs.length; attempt += 1) {
      try { return await mediaFetchBackgroundImageOnce(promptEnriched, w, h); } catch (err) {
        lastErr = err;
  ...
  function mediaGenerateLocalPlaceholder(w, h) {
    ...
    const pal = MEDIA_FALLBACK_PALETTES[Math.floor(Math.random() * MEDIA_FALLBACK_PALETTES.length)];
    ...
    for (let i = 0; i < 4; i += 1) {
      ...
      const bg = ctx.createRadialGradient(bx, by, 0, bx, by, radius);
      bg.addColorStop(0, 'rgba(255,255,255,0.16)'); bg.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.fillStyle = bg; ctx.beginPath(); ctx.arc(bx, by, radius, 0, Math.PI * 2); ctx.fill();
    }
    return canvas;
  }
  async function mediaFetchBackgroundWithFallback(promptEnriched, w, h) {
    try { return { img: await mediaFetchBackgroundImage(promptEnriched, w, h), usedFallback: false }; }
    catch (err) { return { img: mediaGenerateLocalPlaceholder(w, h), usedFallback: true }; }
  }

  // ---------- Composition Canvas 2D (identique au VPS) ----------
  ...
  function mediaWrapText(ctx, text, x, maxWidth, lineHeight, startY, align) {
    ...
    let y = startY; ctx.textAlign = align || 'center';
  ...
  function mediaDrawFlyerLayers(ctx, w, h, fields) {
    ...
    ctx.save();
    ctx.fillStyle = '#fff'; ctx.shadowColor = 'rgba(0,0,0,0.55)'; ctx.shadowBlur = w * 0.01; ctx.textBaseline = 'alphabetic';
  ...
  // Téléchargement — adapté : Capacitor Filesystem + Share (jamais <a download>).
  async function mediaSaveCanvasPng(canvas, filename) {
    try {
      ...
      mediaFeedback("Échec de l'export PNG : " + err.message, true);
    }
  }

  async function mediaSaveBlobViaShare(blob, filename) {
  ...
  function mediaBuildVariantCard(result, index) {
    ...
    if (result.usedFallback) {
      ...
      badge.textContent = '⚠️ Génération IA indisponible — visuel de secours local. Réessayez.';
    ...
    if (!result.ok) { frame.innerHTML = '<p class="empty" style="padding:1rem;color:#c0392b;">Échec de cette proposition.</p>'; return card; }

    const canvas = mediaCreateVariantCanvas(result.img, result.width, result.height, result.deliverable);
    frame.appendChild(canvas);
    const selectBtn = document.createElement('button');
    selectBtn.type = 'button'; selectBtn.textContent = '🟢 Sélectionner';
    selectBtn.addEventListener('click', () => mediaSelectVariant(result));
    actions.appendChild(selectBtn);
    const dlBtn = document.createElement('button');
    dlBtn.type = 'button'; dlBtn.textContent = '📥 Télécharger';
    dlBtn.addEventListener('click', () => mediaSaveCanvasPng(canvas, 'cyrus-visuel-' + Date.now() + '-v' + (index + 1) + '.png'));
  ...
  async function mediaGenerateVariants() {
    ...
    if (!mediaState.ratio) { mediaFeedback('Choisissez un format avant de générer.', true); return; }
    ...
    mediaFeedback('🎨 Génération de ' + MEDIA_VARIANT_STYLES.length + ' propositions en cours (1 à 2 min)...', false);
    ...
    for (let i = 0; i < MEDIA_VARIANT_STYLES.length; i += 1) {
      ...
      if (i > 0) {
        mediaFeedback('🎨 ' + i + '/' + MEDIA_VARIANT_STYLES.length + ' — pause anti-limite...', false);
      ...
      } catch (err) { results.push({ ok: false, label: variantStyle.label }); }
      if (generation !== mediaVariantSeq) return;
    ...
    btn.disabled = false;
    mediaFeedback(successCount === 0 ? 'Échec de génération (service indisponible — réessayez).' : ('✅ ' + successCount + '/' + results.length + ' proposition(s) prête(s).'), successCount === 0);
  }
  ...
  function mediaSelectVariant(result) {
    mediaCurrentCreation = { img: result.img, promptEnriched: result.promptEnriched, width: result.width, height: result.height, ratio: result.ratio, deliverable: result.deliverable, usedFallback: Boolean(result.usedFallback) };
  ...
  async function mediaRegenerateBackgroundWithNote() {
    ...
    try {
      ...
      mediaCurrentCreation.img = img; mediaCurrentCreation.promptEnriched = promptEnriched; mediaCurrentCreation.usedFallback = usedFallback;
      const canvas = document.getElementById('media-canvas');
      const ctx = canvas.getContext('2d');
      mediaDrawCoverImage(ctx, img, mediaCurrentCreation.width, mediaCurrentCreation.height);
      if (mediaCurrentCreation.deliverable === 'FLYER_AFFICHE') mediaDrawFlyerLayers(ctx, mediaCurrentCreation.width, mediaCurrentCreation.height, mediaGetFlyerFields());
      mediaUpdateFallbackWarning();
      mediaFeedback(usedFallback ? '⚠️ Service IA indisponible — fond de secours appliqué.' : '✅ Fond régénéré avec le changement demandé.', usedFallback);
    } catch (err) { mediaFeedback(err.message || 'Erreur lors de la régénération.', true); } finally { btn.disabled = false; }
  }
  async function mediaDeclineAllFormats() {
    ...
    btn.disabled = true; grid.style.display = 'none'; grid.innerHTML = '';
    mediaFeedback('🎞️ Déclinaison en cours (' + otherRatios.length + ' format(s), 1 à 2 min)...', false);
    const results = [];
    for (let i = 0; i < otherRatios.length; i += 1) {
      if (i > 0) { mediaFeedback('🎞️ ' + i + '/' + otherRatios.length + ' — pause anti-limite...', false); await mediaSleep(MEDIA_RATE_LIMIT_MS); }
      ...
      } catch (err) { results.push({ ok: false, ratio }); }
    ...
    results.forEach(({ ok, ratio, canvas, usedFallback }) => {
      ...
      const frame = document.createElement('div');
      card.appendChild(frame);
      if (!ok) { frame.innerHTML = '<p class="empty" style="padding:1rem;color:#c0392b;">Échec pour ce format.</p>'; grid.appendChild(card); return; }
      frame.appendChild(canvas);
      const actions = document.createElement('div');
      actions.className = 'media-card-actions';
      const dlBtn = document.createElement('button');
      dlBtn.type = 'button'; dlBtn.textContent = '📥 Télécharger';
      dlBtn.addEventListener('click', () => mediaSaveCanvasPng(canvas, 'cyrus-visuel-' + ratio.replace(':', 'x') + '-' + Date.now() + '.png'));
      actions.appendChild(dlBtn);
      card.appendChild(actions);
    ...
    const successCount = results.filter((r) => r.ok).length;
    btn.disabled = false;
    mediaFeedback(successCount > 0 ? ('✅ ' + successCount + '/' + otherRatios.length + ' format(s) décliné(s).') : 'Échec de la déclinaison.', successCount === 0);
  }
  ...
  document.getElementById('media-download-png-btn').addEventListener('click', () => {
    mediaSaveCanvasPng(document.getElementById('media-canvas'), 'cyrus-visuel-' + Date.now() + '.png');
  });

  // ---------- Export vidéo Ken Burns (Canvas + MediaRecorder) ----------
  // Support MediaRecorder capturant canvas.captureStream() incertain sur
  // WebView Android (variable selon version) - voir garde try/catch ci-
  // dessous, comportement identique au VPS sinon (même logique de dessin).
  ...
  function mediaDrawSubtitleWord(ctx, word, w, h, scale) {
    ...
    ctx.strokeText(word.toUpperCase(), 0, 0); ctx.fillText(word.toUpperCase(), 0, 0);
    ctx.restore();
  }
  function mediaPickRecorderFormat() {
    const candidates = [
      { mime: 'video/webm;codecs=vp9', ext: 'webm' }, { mime: 'video/webm;codecs=vp8', ext: 'webm' },
  ...
  let mediaLastVideoBlob = null;
  let mediaLastVideoFilename = '';

  async function mediaGenerateVideo() {
    if (!mediaCurrentCreation) { mediaFeedback("Sélectionnez d'abord une création avant d'exporter la vidéo.", true); return; }
    if (mediaCurrentCreation.usedFallback) { mediaFeedback('⚠️ Cette création utilise le fond de secours local — régénérez le fond avant d\'exporter la vidéo.', true); return; }

    let picked;
    try {
      if (!window.MediaRecorder) throw new Error('non supporté');
      picked = mediaPickRecorderFormat();
      if (!picked) throw new Error('aucun format supporté');
    } catch (e) {
      mediaFeedback('Génération vidéo non supportée sur cet appareil (MediaRecorder indisponible dans cette WebView).', true);
      return;
    }
    ...
    mediaFeedback('🎬 Enregistrement en cours — narration audible en aperçu, fichier exporté silencieux.', false);

    let currentWordIndex = -1, boundarySupported = false;
    if (window.speechSynthesis && window.SpeechSynthesisUtterance) {
      try {
        const utterance = new SpeechSynthesisUtterance(scriptText);
        utterance.lang = 'fr-FR'; utterance.rate = voiceRate;
        utterance.onboundary = (ev) => { boundarySupported = true; const spoken = scriptText.slice(0, ev.charIndex).trim(); currentWordIndex = spoken ? spoken.split(/\s+/).length : 0; };
        window.speechSynthesis.cancel(); window.speechSynthesis.speak(utterance);
      } catch (e) { /* synthèse vocale indisponible : export continue sans narration live */ }
    }
    ...
    let recorder, recordedPromise;
    try {
      const stream = canvas.captureStream(60);
      recorder = new MediaRecorder(stream, { mimeType: picked.mime });
      const chunks = [];
      recorder.ondataavailable = (e) => { if (e.data && e.data.size) chunks.push(e.data); };
      recordedPromise = new Promise((resolve) => { recorder.onstop = () => resolve(new Blob(chunks, { type: picked.mime })); });
      recorder.start();
    } catch (err) {
      exportBtn.disabled = false;
      mediaFeedback('Échec du démarrage de l\'enregistrement vidéo sur cet appareil : ' + err.message, true);
      return;
    }
    ...
    let lastWordIndex = -1, wordChangedAt = startTime;
    const POP_MS = 160;
    ...
    await new Promise((resolve) => {
      function frame(now) {
        ...
        mediaDrawSubtitleWord(ctx, words[clampedIdx], width, height, 1.3 - 0.3 * popProgress);
    ...
    exportBtn.disabled = false;
    mediaFeedback('✅ Vidéo exportée (silencieuse, sous-titres incrustés). Vérifiez l\'aperçu avant d\'enregistrer.', false);
  }
  ...
  document.getElementById('media-video-download-btn').addEventListener('click', () => {
    if (!mediaLastVideoBlob) return;
    mediaSaveBlobViaShare(mediaLastVideoBlob, mediaLastVideoFilename || ('cyrus-video-' + Date.now() + '.webm'))
      .catch((err) => mediaFeedback('Échec de l\'enregistrement : ' + err.message, true));
  });
  ...
  const MEDIA_AI_VIDEO_MAX_POLLS = 60;

  async function mediaGenerateAiVideo() {
    ...
    btn.disabled = true;
    mediaFeedback('🤖 Démarrage de la génération vidéo IA...', false);
    ...
    try {
      ...
      if (!result) throw new Error('La génération vidéo IA prend trop de temps — réessayez plus tard.');

      document.getElementById('media-ai-video-preview').src = result.url;
      document.getElementById('media-ai-video-preview-wrap').style.display = 'block';
      window.__cyrusLastAiVideoUrl = result.url;
      mediaFeedback('✅ Vidéo IA générée (fond animé, sans le texte incrusté — voir note en tête de fichier).', false);
    } catch (err) {
      mediaFeedback('❌ ' + err.message, true);
    } finally {
      btn.disabled = false;
    }
  }
  document.getElementById('media-export-ai-video-btn').addEventListener('click', mediaGenerateAiVideo);
  document.getElementById('media-ai-video-download-btn').addEventListener('click', async () => {
    if (!window.__cyrusLastAiVideoUrl) return;
    try {
      const res = await fetch(window.__cyrusLastAiVideoUrl);
      const blob = await res.blob();
      await mediaSaveBlobViaShare(blob, 'cyrus-video-ia-' + Date.now() + '.mp4');
    } catch (err) {
      mediaFeedback('Échec de l\'enregistrement de la vidéo IA : ' + err.message, true);
    }
  });
})();
