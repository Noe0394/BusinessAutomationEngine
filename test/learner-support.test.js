...
test('SERVICE MÉTIER + FORMATION : règles d\'accompagnement portées par le service et injectées au tour d\'apprentissage ; support des ressources sans les charger toutes', async () => {
  ...
  // Seuil relevé une 2e fois (persona client allongée par la règle anti-révélation IA, ~950 caractères, ajoutée le 2026-09-23 sur
  // demande explicite de l'utilisateur — voir personaManager.js#CUSTOMER_NO_AI_REVEAL) : reste une garde réelle contre un
  // chargement non borné des ressources du cours (qui produirait des dizaines de milliers de caractères, pas quelques centaines).
  assert.ok(prompts[prompts.length - 1].length < 11500, 'contexte borné : ' + prompts[prompts.length - 1].length);
});
