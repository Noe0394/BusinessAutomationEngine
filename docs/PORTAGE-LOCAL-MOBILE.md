...
2. Facebook/Messenger PC : capture locale des commentaires des 20 publications recentes et regles de mots-cles ajoutees ci-dessous; restent la capture des messages Messenger entrants sans endpoint public, le raccordement au planificateur commun et la recette Meta. Le telephone a la passerelle securisee et la capture de prospects en code; le D1, l'application Meta, les secrets, l'App Review et le Worker restent a configurer/deployer, puis a valider sur compte reel.
3. Les jobs mobiles de creation/invitation de communautes, avec progression, pause/reprise et journal local, sont ajoutes. Restent leur recette, la verification des ponts officiels sur appareil et l'adhesion WhatsApp par API lorsque celle-ci est disponible et autorisee.
...
5. Valider sur un compte/appareil Android le flux entrant Telegram raccorde a `history_multiappend`. Les reglages globaux du repondeur sont portes. La transcription manuelle d'un fichier audio est implementee; restent l'import automatique des notes vocales entrantes et le cycle de vie de session du repondeur VPS.
6. Porter l'attribution publicitaire verifiee; la capture mobile des prospects Facebook est maintenant dans le code, sous reserve de migration D1, configuration Meta/Cloudflare et recette sur compte reel.
...
- La capture Facebook/Messenger par webhook du VPS est remplacee sur PC par un balayage local opt-in des commentaires des 20 publications recentes, toutes les 5 minutes. Les prospects et regles de mots-cles sont conserves dans SQLite; la reponse automatique par regle reste desactivee par defaut. La capture des messages Messenger entrants et le planificateur commun Page restent absents.
- `mobile/webapp` ne conserve aucun secret Meta; la passerelle serveur autorisee et les commandes Facebook mobile sont implementees dans le Worker et l'application. Le D1, l'application Meta, ses permissions, les secrets et le deploiement sont encore a configurer. La capture mobile des prospects reste absente.
...
Restent en particulier : recettes Meta/Cloudflare et Android, capture des commentaires Messenger entrants et planificateur Page cote PC, journal/couts/dossiers SAV complets dans les rapports mobiles, extraction automatique des notes vocales entrantes et inventaire de parite action par action des 14 onglets VPS. La capture mobile Facebook est implementee en code dans le checkpoint ci-dessous, mais attend migration D1/deploiement Meta et recette. Les integrations dependantes d'une application Meta approuvee, d'un Worker deploye ou d'un appareil reel ne peuvent pas etre declarees validees par le seul code local.

## Capture des prospects Facebook sur telephone - 2026-09-24

L'interface mobile permet d'ajouter/supprimer des regles de mots-cles, de consulter et exporter les prospects, de lancer un scan manuel et d'activer une synchronisation automatique toutes les cinq minutes tant que l'application reste au premier plan. L'opt-in de synchronisation est local a l'appareil; il est coupe a la deconnexion. Le Worker partage les donnees par licence/appareil deja lie et par Page connectee; les regles et prospects sont cloisonnes par Page. Il parcourt au plus 20 publications recentes, lit jusqu'a cinq fils par invocation avec un plafond de huit commentaires bruts, et reprend les fils via des curseurs D1. Les reponses privees par regle sont desactivees par defaut; au plus deux sont traitees par invocation. La capture, les changements de regles, la reconnexion OAuth, le changement de Page et la deconnexion partagent un verrou D1 pour empecher une reponse de partir depuis la mauvaise Page. Une transmission incertaine n'est jamais rejouee automatiquement. Un changement de Page annule les reponses encore en attente et remet a zero les curseurs, et le Worker refuse une capture dont la Page ne correspond plus au compte.

Le schema ajoute `facebook_keyword_rules`, `facebook_leads`, `facebook_seen_comments`, `facebook_capture_state` et `facebook_capture_cursors`. Le schema n'a pas ete applique a un D1 distant; la configuration Meta/Cloudflare et la recette de bout en bout restent a faire. La capture mobile porte sur les commentaires des publications; l'attribution publicitaire Meta verifiee et les messages Messenger entrants restent hors de cette tranche.

Verification locale de la tranche : controles de syntaxe Node, `git diff --check`, synchronisation Capacitor et comparaison des empreintes sources/copies Android. Aucun test automatise, Worker deploye, appel Graph, compte Meta reel ou appareil Android n'a ete utilise.
## Checkpoint passerelle Facebook mobile - 2026-09-24

Le Worker Cloudflare contient une passerelle OAuth/Graph pour Facebook et Messenger. Elle exige une licence active, liee exactement a l'appareil et autorisee pour le module facebook; un jeton ne sort jamais du Worker et est chiffre AES-GCM dans D1. L'ecran telephone permet l'autorisation, le choix de Page, la publication/programming, la lecture des commentaires et conversations, la reponse/moderation et une file Messenger locale persistante avec arret prudent. Les resultats de transport incertains ne sont pas relances automatiquement. La base mobile passe en IndexedDB v12.
...
## Capture Facebook locale sur PC - 2026-09-24

L'onglet Facebook PC propose maintenant des regles de mots-cles, une liste/export Excel des prospects, une synchronisation manuelle et une capture opt-in persistante dans SQLite. Lorsque l'utilisateur l'active et qu'une Page est connectee, le client parcourt toutes les 5 minutes les commentaires des 20 publications recentes; il reprend les commentaires depuis le dernier scan complet et pagine les fils actifs. Les contacts sont dedoublonnes par PSID, les commentaires par identifiant Meta. Chaque regle peut conserver une reponse privee, desactivee par defaut; l'envoi est explicite par regle. Les resultats incertains ne sont pas rejoues automatiquement, y compris apres arret du PC.

Cette voie locale capture les commentaires. Elle ne recoit pas les evenements Messenger entrants du webhook VPS, car le serveur PC n'expose volontairement que 127.0.0.1; la capture des messages Messenger et le planificateur commun de publications Facebook Page restent a porter. La capture PC couvre les 20 publications recentes; les autorisations Meta et l'App Review doivent permettre la lecture de commentaires et `pages_messaging` pour les reponses privees. Une reponse automatique suit les limites appliquees par Meta; un refus ou une coupure reseau reste marque incertain sans renvoi. La capture mobile est ajoutee dans le checkpoint telephone ci-dessus.

Verification locale de cette tranche : `node --check` sur le serveur, la base SQLite, l'adaptateur Facebook et l'interface; `git diff --check` reussi. Aucun test automatise ni appel Meta reel n'a ete execute. L'etat local est encore non valide sur un compte Page reel.
