...
// ADAPTATEUR local-client (2026-09-22) : pas de gestionnaire multi-tenant (un seul compte local par canal) —
// require('../adapters/{whatsapp,telegram}Manager').getOrCreate(tenant).session devient require('../lib/
// {whatsapp,telegram}') directement. lib/whatsapp.js a été étendu (checkNumbersOnWhatsApp, joinGroupByInvite,
// getInviteInfo...) pour porter ce fichier — NON TESTÉ en conditions réelles.
...
async function searchWhatsApp(tenant, keywords, limit, deps, locationSuffix) {
  ...
  let session = null;
  try { session = require('../lib/whatsapp'); } catch (e) { session = null; }
