    ...
    async execute(args, ctx) {
      ...
      try {
        ...
        if (!groupId && groupName) {
          // ADAPTATEUR local-client : pas de gestionnaire multi-tenant ici (un seul compte local par canal) —
          // ai-engine/toolsExtra.js (VPS) passe par adapters/{whatsapp,telegram}Manager.getOrCreate(tenant).session ;
          // ici, lib/whatsapp.js et lib/telegram.js SONT directement la session (mono-compte).
          const session = channel === 'TELEGRAM' ? require('../lib/telegram') : require('../lib/whatsapp');
