...
// ADAPTATEUR local-client : pas de gestionnaire multi-tenant (un seul compte local par canal) — `session` est le
// module lib/whatsapp.js ou lib/telegram.js directement, jamais un peek() sur un registre de tenants.
const connected = (session) => { try { return !!(session && typeof session.isConnected === 'function' && session.isConnected()); } catch (e) { return false; } };
...
const PLANS = {
  ...
  'connect-whatsapp': {
    ...
    steps: [{ id: 'paired', title: 'Scanner le QR code ou saisir le code d\'association', instruction: 'Ouvrez « Connexions & Intégrations », bloc WhatsApp, et scannez le QR code depuis WhatsApp > Appareils connectés (ou utilisez le code d\'association).', tools: [], check: async (t, deps) => yes(deps && deps.connected ? deps.connected.WHATSAPP(t) : connected(require('../lib/whatsapp')), 'connexion WhatsApp') }],
  },
  'connect-telegram': {
    ...
    steps: [{ id: 'logged', title: 'Vous connecter avec votre numéro et le code Telegram', instruction: 'Dans « Connexions & Intégrations », bloc Telegram : saisissez votre numéro puis le code reçu.', tools: [], check: async (t, deps) => yes(deps && deps.connected ? deps.connected.TELEGRAM(t) : connected(require('../lib/telegram')), 'connexion Telegram') }],
  },
  'configure-sav': {
