// ADAPTATEUR local-client : resynchronisé verbatim depuis le VPS le 2026-09-23 (débloquait le Chat Intelligent local —
// voir docs/PORTAGE-LOCAL-MOBILE.md, « piège authz/principal »). UNE seule adaptation nécessaire dans tout ce fichier :
// searchMyGroups() ci-dessous (pas de gestionnaire multi-tenant en mode local mono-compte, voir toolsExtra.js pour le
// même patron). Ne PAS réappliquer un `cp` aveugle depuis le VPS sans repasser sur cette adaptation.
...
async function searchMyGroups(channel, { adminOnly, subject } = {}, tenantId, deps) {
  ...
  if (subjName && channel === 'WHATSAPP' && top.length <= 5) {
    try {
      // ADAPTATEUR local-client : pas de gestionnaire multi-tenant (un seul compte local) — voir toolsExtra.js pour le même patron.
