...
async function capabilities(tenant) {
  ...
  // ADAPTATEUR local-client : pas de gestionnaire multi-tenant (un seul compte local par canal) — `session` est
  // directement lib/whatsapp.js ou lib/telegram.js, jamais un peek() sur un registre de tenants.
  const state = (session) => { try { return !!(session && typeof session.isConnected === 'function' && session.isConnected()); } catch (e) { return false; } };
  const channels = { WHATSAPP: state(require('../lib/whatsapp')), TELEGRAM: state(require('../lib/telegram')), WEB: true };
