...
(function (root, factory) {
  ...
  const ACTIONS = ['EXTRACT_MEMBERS', 'SEND_CAMPAIGN', 'FOLLOW_UP', 'ANALYZE_HUMAN_CONTEXT',
    'ANALYZE_RESPONSES', 'REPLY_COMMENT', 'GENERATE_VIDEO', 'PAUSE_CAMPAIGN',
    'RESUME_CAMPAIGN', 'GENERATE_REPORT', 'CREATE_USER_ACCOUNT', 'GENERATE_ACCESS_KEY',
    'SCHEDULE_FOLLOWUP', 'PROVIDE_PAYMENT_INSTRUCTIONS', 'GENERATE_PAYMENT_LINK', 'NEGOTIATE_DISCOUNT',
  ...
  function createActionExecutor(deps) {
    ...
    registry.CREATE_USER_ACCOUNT = async (payload, meta) => {
      ...
      const tenantId = (meta && meta.tenantId) || (payload && payload.tenantId) || null;
    ...
    registry.GENERATE_ACCESS_KEY = async (payload, meta) => {
      const k = generateAccessKey(payload);
      if (runtime.generateAccessKey) {
        const tenantId = (meta && meta.tenantId) || (payload && payload.tenantId) || null;
        if (!tenantId) return { ok: false, error: 'TENANT_REQUIRED' };
        const ext = await runtime.generateAccessKey(Object.assign({}, payload || {}, { tenantId }), meta).catch((e) => ({ ok: false, error: String(e && e.message || e) }));
        if (!ext || ext.ok !== true) return { ok: false, error: (ext && ext.error) || 'LICENSE_ISSUE_FAILED' };
    ...
    registry.SCHEDULE_FOLLOWUP = async (payload, meta) => {
      ...
      if (runtime.scheduleFollowUp) {
        const res = await runtime.scheduleFollowUp(Object.assign({}, payload, { channel, tenantId }));
    ...
    // -------- 14. Instructions de paiement manuel (aucune URL de paiement) ---------------------------------------
    // Cyrus ne sait pas générer de vrai lien de paiement (aucun agrégateur CinetPay/PayDunya/Stripe...) : ce message est un simple récapitulatif
    // formaté des moyens de paiement RÉELLEMENT configurés par le vendeur, avec une référence unique à rapprocher manuellement.
    // SOURCE DE VÉRITÉ : le(s) Service(s) métier du tenant (commercial.paymentTerms), via `d.getPaymentMethods(tenantId)` — INJECTÉ (ce module reste
    // dual-env et ne requiert jamais businessServices.js, qui dépend de fs/storageAdapter côté Node). Repli sur les anciennes variables d'environnement
    // globales UNIQUEMENT si ce hook n'est pas fourni (compatibilité d'autres intégrations à un seul compte) — jamais utilisé sur le VPS multi-tenant.
    registry.PROVIDE_PAYMENT_INSTRUCTIONS = async (payload) => {
      ...
      let operators = [];
      if (typeof d.getPaymentMethods === 'function') {
        try { operators = (await d.getPaymentMethods(payload && payload.tenantId)) || []; } catch (e) { operators = []; }
        operators = operators.filter((o) => o && o.label && o.value).map((o) => [o.label, o.value]);
      } else {
        operators = [
      ...
      if (!operators.length) {
        return { ok: false, error: 'NO_PAYMENT_METHOD_CONFIGURED' };
      }

      // Pas de ligne "Bénéficiaire" générique séparée : le titulaire réel, quand le vendeur l'a précisé, est déjà
      // repris TEL QUEL dans chaque `value` (paymentTerms) ci-dessus — une ligne globale (env
      // MOBILE_MONEY_BENEFICIARY_NAME ou un nom de plateforme par défaut) afficherait un nom potentiellement FAUX
      // pour ce tenant précis (le paiement doit aller au vendeur réel, jamais à un nom générique inventé/partagé).
    ...
    // Compatibilité des tâches déjà enregistrées : cet ancien nom retourne les
    // mêmes coordonnées manuelles et ne génère ni ne transmet jamais d'URL.
    registry.GENERATE_PAYMENT_LINK = registry.PROVIDE_PAYMENT_INSTRUCTIONS;
    ...
    // Même route Cloudflare que CREATE_USER_ACCOUNT mais account.create:false — l'étudiant
    ...
    // qui a déjà payé.
    registry.GRANT_MODULE_ACCESS = async (payload, meta) => {
