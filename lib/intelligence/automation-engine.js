...
//   { id, runId, tenantId, engine: 'ZERO_VPS'|'VPS_BAILEYS'|'WHATSAPP_LOCAL',
...
(function (root, factory) {
  ...
  const ENGINE_IDS = ['ZERO_VPS', 'VPS_BAILEYS', 'WHATSAPP_LOCAL'];
  ...
  function createAutomationEngine(deps) {
    ...
    async function hydrateFromStorage() {
      ...
      const storedRuns = Array.isArray(data.runs) ? data.runs : [];
      const resumed = storedTasks.reduce((n, t) => {
        const s = store._internal && store._internal.tasks;
        if (s) s.set(t.id, t);
        if (t.status === 'processing') { t.status = 'queued'; t.attempts = 0; n++; }
        return n;
      }, 0);
      if (store.markRunBatch && storedRuns.length) await store.markRunBatch(storedRuns);
      return { resumed, alreadyRun: storedRuns.length };
    }
    ...
    async function executeOne(t, now, hints) {
      ...
      try {
        if (!executor) {
          ...
          // Le tenant stocké sur la tâche fait autorité. Un tenantId placé
          // dans le payload par un appelant ne peut pas rediriger la session.
          outcome = await executor.execute(t.type, Object.assign({}, t.payload || {}, { tenantId: t.tenantId }), {
