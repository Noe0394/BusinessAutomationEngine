...
function createVpsBridge(opts) {
  ...
  router.post('/api/intelligence/goal-chat', async (req, res) => {
    ...
    const orchestrated = await chatOrchestrator.handle(
      { text: message, history: [], tenantId: tenant, sessionId: state.sessionId, lastAssistantMessage },
    ...
    if (!looksLikeRealGoal) {
      try {
        ...
        const { text: raw } = await llmFallbackEngine.generateAIResponse(prompt, [], null);
