...
function createVpsBridge(opts) {
  ...
  function persistSession(tenant, sessionId) {
    const key = sessionKey(tenant, sessionId);
    const messages = sessionHistory.get(key) || [];
    chatMemoryStore.set(CHAT_MEMORY_NAMESPACE, memDocId(tenant, sessionId), {
  ...
  function recordTurn(tenant, sessionId, userText, assistantText, meta) {
    const key = sessionKey(tenant, sessionId);
  ...
  function clearHistory(tenant, sessionId) {
    ...
    chatMemoryStore.set(CHAT_MEMORY_NAMESPACE, memDocId(tenant, sessionId), { tenant: memSanitize(tenant), sessionId, messages: [], context: {}, updatedAt: new Date().toISOString() });
  }
  ...
  router.post('/api/intelligence/goal-chat', async (req, res) => {
    ...
    const lastAssistantMessage = lastAssistantBySession.get(key) || null;
    const orchestrated = await chatOrchestrator.handle(
      { text: message, history: await getHistory(tenant, state.sessionId), tenantId: tenant, sessionId: state.sessionId, lastAssistantMessage, principal: principalFor(req, tenant), tainted },
    ...
    if (orchestrated) {
      ...
      lastAssistantBySession.set(key, {
  ...
  router.delete('/api/intelligence/conversations/:sessionId', async (req, res) => {
    const tenant = tenantForRequest(req, req.query.tenantId);
    clearHistory(tenant, req.params.sessionId);
