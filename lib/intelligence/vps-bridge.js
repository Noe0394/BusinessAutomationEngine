...
function createVpsBridge(opts) {
  ...
  // MÉMOIRE DE CONVERSATION par session (en mémoire, non persistante) — l'onglet
  // "Chat Intelligent" ne stockait aucun historique (contrairement au Copywriter
  // Studio IA via aiStudioStore), donc l'agent répondait sans aucune mémoire du
  // fil. On garde un historique borné {role,text} pour le fournir au moteur et
  // au repli conversationnel, afin qu'il se souvienne de l'échange en cours.
  ...
  router.post('/api/intelligence/goal-chat', async (req, res) => {
    ...
    if (action === 'restart') {
      ...
      sessionHistory.delete(state.sessionId);
    ...
    const orchestrated = await chatOrchestrator.handle(
      { text: message, history: getHistory(state.sessionId), tenantId: tenant, sessionId: state.sessionId, lastAssistantMessage },
      { runtime: deps.runtime || null, engineFor, humanContext },
    ).catch((err) => {
    ...
    if (orchestrated) {
      ...
      recordTurn(state.sessionId, message, orchestrated.text);
      return res.json({
        ok: true,
    ...
    if (!looksLikeRealGoal) {
      try {
        ...
        const prompt = `${personaManager.personaSystemPrompt('default')}\nMessage du vendeur : "${message}"`;
        const { text: raw } = await llmFallbackEngine.generateAIResponse(prompt, getHistory(state.sessionId), null);
        recordTurn(state.sessionId, message, raw);
    ...
    recordTurn(state.sessionId, message, out && out.reply && out.reply.text);
