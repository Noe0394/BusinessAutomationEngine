...
function createVpsBridge(opts) {
  ...
  const sessionKey = (tenant, sessionId) => JSON.stringify([String(tenant || ''), String(sessionId || '')]);
  // Registre d'outils central (pont technique intelligence ↔ fonctionnalités).
  const toolRegistry = deps.toolRegistry || require('../../ai-engine/toolRegistry');
  const authz = require('../../ai-engine/authz');
  const untrusted = require('../../ai-engine/untrusted');
  ...
  // IDENTITÉ AUTHENTIFIÉE de l'appelant (jamais déduite d'un texte ni d'un champ du corps de requête) : le middleware d'accès a
  // marqué la requête (mot de passe administrateur => req.isAdmin ; clé de licence valide => req.licenseKey). Une clé de licence
  // n'opère QUE sur son propre compte. Sans marqueur d'authentification : aucun principal, donc aucun outil.
  function principalFor(req, tenant) {
    if (req.isAdmin === true) return authz.issuePrincipal({ tenant, role: authz.ROLES.ADMIN, channel: 'WEB', via: 'web_admin', allowedModules: req.allowedModules });
    if (req.licenseKey && String(req.licenseKey) === String(tenant)) return authz.issuePrincipal({ tenant, role: authz.ROLES.OWNER, userId: tenant, channel: 'WEB', via: 'web_license', allowedModules: req.allowedModules });
    return null;
  }

  function toolContextFor(req, tenant) {
    const supplied = typeof deps.toolContext === 'function' ? deps.toolContext(tenant, req) : (deps.toolContext || {});
    return Object.assign({}, supplied || {}, { tenant, allowedModules: req.allowedModules });
  }

  function deniedTaskModule(req, tasks) {
    if (req.allowedModules === null) return null;
    if (!Array.isArray(req.allowedModules)) return 'whatsapp';
    for (const task of tasks || []) {
      const type = String(task.type || task.action || '').toUpperCase();
      if (type === 'GENERATE_VIDEO' && !req.allowedModules.includes('studio_video')) return 'studio_video';
      const channel = String(task.channel || (task.payload && task.payload.channel) || '').toUpperCase();
      const module = channel === 'TELEGRAM' ? 'telegram' : (channel === 'WHATSAPP' ? 'whatsapp' : null);
      if (module && !req.allowedModules.includes(module)) return module;
    }
    return null;
  }
  ...
  // par (tenant, sessionId) — même compromis de persistance que `engines` ci-dessous
  ...
  const sessionHistory = new Map(); // cache: (tenant, sessionId) -> [{role,text}]
  ...
  const sessionContext = new Map(); // cache: (tenant, sessionId) -> contexte persistant
  const sessionTitle = new Map(); // cache: (tenant, sessionId) -> titre (1er message)
  function msgId() { return 'm_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6); }
  async function getHistory(tenant, sessionId) {
    const key = sessionKey(tenant, sessionId);
    if (sessionHistory.has(key)) return sessionHistory.get(key);
    const doc = await chatMemoryStore.get(CHAT_MEMORY_NAMESPACE, memDocId(tenant, sessionId), { messages: [] });
    const messages = Array.isArray(doc.messages) ? doc.messages : [];
    sessionHistory.set(key, messages);
    if (doc.context) sessionContext.set(key, doc.context);
    if (doc.title) sessionTitle.set(key, doc.title);
    return messages;
  }
  async function persistSession(tenant, sessionId) {
    const key = sessionKey(tenant, sessionId);
    const messages = sessionHistory.get(key) || [];
    return chatMemoryStore.setDurable(CHAT_MEMORY_NAMESPACE, memDocId(tenant, sessionId), {
      tenant: memSanitize(tenant), sessionId,
      title: sessionTitle.get(key) || null,
      context: sessionContext.get(key) || {},
      messages, updatedAt: new Date().toISOString(),
    });
  }
  // Enregistre un tour de conversation avec métadonnées RÉELLES persistantes :
  // chaque message porte id, rôle, content (+ text pour compat), timestamp et
  // metadata (ex. tool call effectué). Survit à un redémarrage ; effaçable via
  // 'restart'. `meta` (facultatif) est attaché au message assistant.
  async function recordTurn(tenant, sessionId, userText, assistantText, meta) {
    const key = sessionKey(tenant, sessionId);
    const h = sessionHistory.get(key) || [];
    const now = new Date().toISOString();
    if (!sessionTitle.get(key) && userText) sessionTitle.set(key, String(userText).slice(0, 60));
    h.push({ id: msgId(), role: 'user', content: String(userText || ''), text: String(userText || ''), ts: now, metadata: {} });
    if (assistantText) h.push({ id: msgId(), role: 'assistant', content: String(assistantText).slice(0, 4000), text: String(assistantText).slice(0, 4000), ts: new Date().toISOString(), metadata: meta || {} });
    while (h.length > 60) h.shift();
    sessionHistory.set(key, h);
    await persistSession(tenant, sessionId);
  }
  function getSessionContext(tenant, sessionId) { return sessionContext.get(sessionKey(tenant, sessionId)) || {}; }
  function setSessionContext(tenant, sessionId, patch) {
    const key = sessionKey(tenant, sessionId);
    const cur = sessionContext.get(key) || {};
    const next = Object.assign({}, cur, patch || {});
    sessionContext.set(key, next);
    persistSession(tenant, sessionId);
    return next;
  }
  // Efface la mémoire d'une discussion (à la demande de l'utilisateur, via
  // 'restart') : cache + document persistant vidés.
  async function clearHistory(tenant, sessionId) {
    const key = sessionKey(tenant, sessionId);
    sessionHistory.delete(key);
    sessionContext.delete(key);
    sessionTitle.delete(key);
    await chatMemoryStore.setDurable(CHAT_MEMORY_NAMESPACE, memDocId(tenant, sessionId), { tenant: memSanitize(tenant), sessionId, messages: [], context: {}, updatedAt: new Date().toISOString() });
  }
  // Liste les conversations persistées d'un tenant (pour l'onglet Chat : y
  // revenir et continuer). S'appuie sur storageAdapter.listIds filtré par le
  // préfixe du tenant.
  async function listConversations(tenant) {
    const prefix = memSanitize(tenant) + '__';
    let ids = [];
    try { ids = await chatMemoryStore.listIds(CHAT_MEMORY_NAMESPACE); } catch (e) { ids = []; }
    const mine = (ids || []).filter((id) => String(id).startsWith(prefix));
    const out = [];
    for (const id of mine) {
      const doc = await chatMemoryStore.get(CHAT_MEMORY_NAMESPACE, id, null);
      if (!doc || !Array.isArray(doc.messages) || !doc.messages.length) continue;
      out.push({
        sessionId: doc.sessionId || id.slice(prefix.length),
        title: doc.title || (doc.messages[0] && (doc.messages[0].content || doc.messages[0].text)) || 'Conversation',
  ...
  function runtimeExecutorFor(tenantId) {
    ...
    return {
      ...
      actionExecutor: actionExecutorMod.createActionExecutor({
        ...
        // Moyens de paiement RÉELLEMENT configurés dans le(s) Service(s) métier du compte (jamais une variable d'environnement globale partagée
        // entre tenants) — voir GENERATE_PAYMENT_LINK dans action-executor.js. Chargé ici (Node) pour garder ce module dual-env sans dépendance directe.
        getPaymentMethods: async (t) => {
          try {
            const businessServices = require('../../ai-engine/businessServices');
            const list = await businessServices.list(t || tenantId);
            return list.filter((s) => (s.lifecycle || 'active') === 'active' && s.commercial && String(s.commercial.paymentTerms || '').trim())
              .map((s) => ({ label: s.name, value: businessServices.stripPaymentUrls(s.commercial.paymentTerms) }))
              .filter((s) => s.value);
          } catch (e) { return []; }
        },
      }),
  ...
  router.post('/api/intelligence/objective', (req, res) => {
    ...
    const tenant = (req.isAdmin === true && tenantId) ? String(tenantId) : tenantForRequest(req, tenantId);
  ...
  router.post('/api/intelligence/execute', async (req, res) => {
    ...
    const tenant = (req.isAdmin === true && tenantId) ? String(tenantId) : tenantForRequest(req, tenantId);
    const principal = principalFor(req, tenant);
    if (!principal) return res.status(401).json({ error: 'Authentification requise pour ce compte.' });
    const actionName = String(action).toUpperCase();
    if (!actionExecutorMod.ACTIONS.includes(actionName)) return res.status(400).json({ error: 'Action inconnue.' });
    if (['CREATE_USER_ACCOUNT', 'GENERATE_ACCESS_KEY', 'GRANT_MODULE_ACCESS'].includes(actionName) && req.isAdmin !== true) {
    ...
    const requiredModule = actionName === 'GENERATE_VIDEO' ? 'studio_video'
      : ['WHATSAPP', 'TELEGRAM'].includes(channel) && ['EXTRACT_MEMBERS', 'SEND_CAMPAIGN', 'FOLLOW_UP', 'REPLY_COMMENT',
        'PAUSE_CAMPAIGN', 'RESUME_CAMPAIGN', 'SCHEDULE_FOLLOWUP', 'READ_RECENT_MESSAGES', 'LIST_GROUPS'].includes(actionName)
        ? channel.toLowerCase() : null;
    if (requiredModule && req.allowedModules !== null
        && (!Array.isArray(req.allowedModules) || !req.allowedModules.includes(requiredModule))) {
      return res.status(403).json({ error: `Votre clé de licence n'inclut pas le module "${requiredModule}".` });
    }
    const eng = engineFor(tenant);
    const task = await eng.createTask({
      runId: runId || null,
      tenantId: tenant,
      engine: engine || 'VPS_BAILEYS',
      type: actionName,
      payload: Object.assign({}, body, { tenantId: tenant }),
      scheduledAt: Date.now(),
    });
    const out = await authz.runAs(principal, () => eng.runDue({ engine: engine || 'VPS_BAILEYS', tenantId: tenant }));
    res.json({ ok: true, taskId: task.id, out, executorRuntime: !!deps.runtime });
  });
  ...
  router.post('/api/intelligence/chat', async (req, res) => {
    ...
    const tenant = (req.isAdmin === true && tenantId) ? String(tenantId) : tenantForRequest(req, tenantId);
    const principal = principalFor(req, tenant);
    if (!principal) return res.status(401).json({ error: 'Authentification requise.' });
    const doc = taskParser.parseObjective({
      text: message,
      engine: engine || 'VPS_BAILEYS',
      tenantId: tenant,
    ...
    const deniedModule = deniedTaskModule(req, tasks);
    if (deniedModule) return res.status(403).json({ error: `Votre licence n'inclut pas le module "${deniedModule}".`, module: deniedModule });
    const eng = engineFor(tenant);
    await eng.createTasks(tasks);
    const executed = await authz.runAs(principal, () => eng.runDue({ engine: engine || 'VPS_BAILEYS', tenantId: tenant }));
    res.json({
      ok: true,
      tenantId: tenant,
      runId,
  ...
  router.post('/api/intelligence/goal-chat', async (req, res) => {
    const { sessionId, tenantId, action, attachments } = req.body || {};
    let message = (req.body && typeof req.body.message === 'string') ? req.body.message : '';
    const tenant = (req.isAdmin === true && tenantId) ? String(tenantId) : tenantForRequest(req, tenantId);
    const requestedKey = sessionId ? sessionKey(tenant, sessionId) : null;

    let state = requestedKey ? goalChatSessions.get(requestedKey) : null;
    if (!state) {
      state = goalChat.createSession({});
      goalChatSessions.set(sessionKey(tenant, state.sessionId), state);
    }

    if (action === 'restart') {
      goalChatSessions.delete(sessionKey(tenant, state.sessionId));
      await clearHistory(tenant, state.sessionId);
      lastAssistantBySession.delete(sessionKey(tenant, state.sessionId));
      const fresh = goalChat.createSession({});
      goalChatSessions.set(sessionKey(tenant, fresh.sessionId), fresh);
    ...
    if (action === 'run-plan') {
      ...
      const principal = principalFor(req, tenant);
      if (!principal) return res.status(401).json({ error: 'Authentification requise.' });
      const deniedModule = deniedTaskModule(req, state.tasks);
      if (deniedModule) return res.status(403).json({ error: `Votre licence n'inclut pas le module "${deniedModule}".`, module: deniedModule });
      const result = await goalChat.runPlan(state, {
        execute: async (tasks) => {
          const deniedModule = deniedTaskModule(req, tasks);
          if (deniedModule) throw Object.assign(new Error(`Votre licence n'inclut pas le module "${deniedModule}".`), { code: 'MODULE_NOT_ALLOWED', module: deniedModule });
          await eng.createTasks(tasks);
          const out = await authz.runAs(principal, () => eng.runDue({ tenantId: tenant }));
          return {
      ...
      if (result && result.error && result.error.code === 'MODULE_NOT_ALLOWED') return res.status(403).json({ error: result.error.message, code: result.error.code, module: result.error.module });
      return res.json(Object.assign({ ok: result.ok }, goalChat.serialize(state, { mode: 'run' })));
    }

    // Pièces jointes importées dans la discussion : on injecte leur contexte
    // RÉEL (contenu des fichiers texte ; simple signalement pour le binaire, sans
    // invention) dans le message vu par l'IA. Un message vide + une pièce jointe
    // reste valide (la pièce jointe est le contenu).
    let tainted = false;
    if (attachments && attachments.length) {
      try {
        const attCtx = await require('../../ai-engine/chatUploads').buildContext(tenant, attachments);
        if (attCtx) { message = (message ? message + '\n\n' : '') + attCtx + '\n\n' + untrusted.GUARD_INSTRUCTION; tainted = true; }
      } catch (e) { /* non bloquant */ }
      if (!message) message = '[pièce(s) jointe(s) importée(s)]';
    }
    ...
    const key = sessionKey(tenant, state.sessionId);
    const history = await getHistory(tenant, state.sessionId);
    const lastStoredAssistant = [...history].reverse().find((item) => item && item.role === 'assistant');
    const storedPending = lastStoredAssistant && lastStoredAssistant.metadata && lastStoredAssistant.metadata.pendingState;
    const lastAssistantMessage = lastAssistantBySession.get(key) || (lastStoredAssistant ? Object.assign({
      role: 'assistant',
      isPlanningQuestion: !!(storedPending && storedPending.isPlanningQuestion),
      intent: storedPending && storedPending.intent || lastStoredAssistant.metadata.intent || null,
    }, storedPending || {}) : null);
    const orchestrated = await chatOrchestrator.handle(
      { text: message, history, tenantId: tenant, sessionId: state.sessionId, lastAssistantMessage, principal: principalFor(req, tenant), tainted },
      { runtime: deps.runtime || null, engineFor, humanContext, generateImage: deps.generateImage || null, toolContext: toolContextFor(req, tenant) },
    ).catch((err) => {
    ...
    if (orchestrated) {
      // JAMAIS « fait » sans preuve d'exécution dans ce tour (ai-engine/claimGuard.js).
      if (orchestrated.text) orchestrated.text = require('../../ai-engine/claimGuard').guard(orchestrated.text, orchestrated, { request: message }).text;
      const pendingState = {
        isPlanningQuestion: !!orchestrated.isPlanningQuestion,
        intent: orchestrated.intent || null,
      };
      if (orchestrated.adDraft) pendingState.adDraft = orchestrated.adDraft;
      if (orchestrated.gcDraft) pendingState.gcDraft = orchestrated.gcDraft;
      // Planning state is produced by server-side handlers. Bound it before
      // storing it with the conversation so an attachment or long campaign
      // draft cannot grow the durable session without limit.
      let boundedPendingState = pendingState;
      try {
        if (Buffer.byteLength(JSON.stringify(pendingState), 'utf8') > 24000) {
          boundedPendingState = { isPlanningQuestion: pendingState.isPlanningQuestion, intent: pendingState.intent };
        }
      } catch (_) { boundedPendingState = { isPlanningQuestion: pendingState.isPlanningQuestion, intent: pendingState.intent }; }
      lastAssistantBySession.set(key, Object.assign({
        isPlanningQuestion: !!orchestrated.isPlanningQuestion,
        intent: orchestrated.intent || null,
      }, orchestrated.adDraft ? { adDraft: orchestrated.adDraft } : {}, orchestrated.gcDraft ? { gcDraft: orchestrated.gcDraft } : {}));
      await recordTurn(tenant, state.sessionId, message, orchestrated.text, {
        intent: orchestrated.intent || null,
        toolCall: orchestrated.toolCall || null,
        actionLog: orchestrated.actionLog || null,
        pendingState: boundedPendingState,
      });
      return res.json({
        ok: true,
    ...
    if (!looksLikeRealGoal) {
      try {
        ...
        const { text: rawLlm } = await llmFallbackEngine.generateAIResponse(prompt, await getHistory(tenant, state.sessionId), null);
        const raw = require('../../ai-engine/claimGuard').guard(rawLlm, null, { request: message }).text; // conversation libre = aucune preuve d'exécution
        await recordTurn(tenant, state.sessionId, message, raw);
    ...
    await recordTurn(tenant, state.sessionId, message, out && out.reply && out.reply.text);
  ...
  // --- Tool Registry : pont technique central intelligence ↔ fonctionnalités.
  // Liste des outils réellement disponibles (contrat complet) et exécution
  // vérifiée d'un tool call, avec états explicites PENDING/RUNNING/SUCCESS/
  // FAILED/BLOCKED/UNCONFIRMED. Le propriétaire authentifié (son propre compte)
  // a par défaut le droit d'envoi ; on ne restreint que si `permissions` est
  // fourni. Le runtime réel (envoi WhatsApp/Telegram vérifié) est injecté.
  router.get('/api/intelligence/tools', (req, res) => {
    const tenant = tenantForRequest(req);
    const principal = principalFor(req, tenant);
    if (!principal) return res.status(401).json({ error: 'Authentification requise.' });
    res.json({ ok: true, tools: toolRegistry.list({ principal, allowedModules: req.allowedModules }) });
  });
  router.post('/api/intelligence/tool-call', async (req, res) => {
    const { name, args, tenantId } = req.body || {};
    if (!name || typeof name !== 'string') return res.status(400).json({ error: 'Le champ "name" (outil) est requis.' });
    // L'admin (propriétaire) peut cibler explicitement un tenant (ex. un compte
    // sous clé de licence) pour opérer/tester dessus ; sinon on résout le tenant
    // depuis l'identité authentifiée (une clé de licence n'opère qu'elle-même).
    const tenant = (req.isAdmin && tenantId) ? tenantId : tenantForRequest(req, tenantId);
    const principal = principalFor(req, tenant);
    if (!principal) return res.status(401).json({ error: 'Authentification requise.' });
    const out = await authz.runAs(principal, () => toolRegistry.execute(tenant, name, args || {}, Object.assign(toolContextFor(req, tenant), { runtime: deps.runtime || null, principal })));
    res.json({ ok: out.state === 'SUCCESS', call: out });
  });

  // --- Mémoire persistante du chat : lister / recharger / supprimer une
  // conversation (l'utilisateur revient dessus et continue sur le même projet).
  router.get('/api/intelligence/conversations', async (req, res) => {
    const tenant = tenantForRequest(req, req.query.tenantId);
    res.json({ ok: true, conversations: await listConversations(tenant) });
  });
  router.get('/api/intelligence/conversations/:sessionId', async (req, res) => {
    const tenant = tenantForRequest(req, req.query.tenantId);
    const messages = await getHistory(tenant, req.params.sessionId);
    res.json({ ok: true, sessionId: req.params.sessionId, messages, context: getSessionContext(tenant, req.params.sessionId), title: sessionTitle.get(sessionKey(tenant, req.params.sessionId)) || null });
  });
  router.delete('/api/intelligence/conversations/:sessionId', async (req, res) => {
    const tenant = tenantForRequest(req, req.query.tenantId);
    await clearHistory(tenant, req.params.sessionId);
    res.json({ ok: true, removed: req.params.sessionId });
  });
