...
function createVpsBridge(opts) {
  ...
  function principalFor(req, tenant) {
    if (req.isAdmin === true) return authz.issuePrincipal({ tenant, role: authz.ROLES.ADMIN, channel: 'WEB', via: 'web_admin' });
    if (req.licenseKey && String(req.licenseKey) === String(tenant)) return authz.issuePrincipal({ tenant, role: authz.ROLES.OWNER, userId: tenant, channel: 'WEB', via: 'web_license' });
    return null;
  }
  ...
  const sessionContext = new Map(); // cache: sessionId -> contexte persistant
  const sessionTitle = new Map(); // cache: sessionId -> titre (1er message)
  ...
  async function getHistory(tenant, sessionId) {
    ...
    if (doc.context) sessionContext.set(sessionId, doc.context);
    if (doc.title) sessionTitle.set(sessionId, doc.title);
    return messages;
  }
  function persistSession(tenant, sessionId) {
    ...
    chatMemoryStore.set(CHAT_MEMORY_NAMESPACE, memDocId(tenant, sessionId), {
      ...
      title: sessionTitle.get(sessionId) || null,
  ...
  function recordTurn(tenant, sessionId, userText, assistantText, meta) {
    ...
    if (!sessionTitle.get(sessionId) && userText) sessionTitle.set(sessionId, String(userText).slice(0, 60));
  ...
  function getSessionContext(sessionId) { return sessionContext.get(sessionId) || {}; }
  function setSessionContext(tenant, sessionId, patch) {
    const cur = sessionContext.get(sessionId) || {};
    const next = Object.assign({}, cur, patch || {});
    sessionContext.set(sessionId, next);
    persistSession(tenant, sessionId);
    return next;
  }
  ...
  router.get('/api/intelligence/tools', (req, res) => {
    res.json({ ok: true, tools: toolRegistry.describe() });
  });
  router.post('/api/intelligence/tool-call', async (req, res) => {
    ...
    const out = await authz.runAs(principal, () => toolRegistry.execute(tenant, name, args || {}, { runtime: deps.runtime || null }));
  ...
  router.get('/api/intelligence/conversations/:sessionId', async (req, res) => {
    ...
    res.json({ ok: true, sessionId: req.params.sessionId, messages, context: getSessionContext(req.params.sessionId), title: sessionTitle.get(req.params.sessionId) || null });
  });
