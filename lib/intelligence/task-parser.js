...
(function (root, factory) {
  ...
  const ACTIONS = [
    ...
    'SCHEDULE_FOLLOWUP', 'GENERATE_PAYMENT_LINK', 'NEGOTIATE_DISCOUNT', 'GRANT_MODULE_ACCESS',
  ];
  ...
  function detectGoals(text, amounts) {
    ...
    // PAIEMENT / remise (lien de paiement Mobile Money, négociation de prix)
    ...
    for (const goal of goals) {
      switch (goal.type) {
        ...
        case 'PAYMENT': {
          ...
          plan.push(desc('GENERATE_PAYMENT_LINK', channels[0], { tenantId: user.tenantId, engine: user.engine }, user, { priority: 60, immediate: true }));
          break;
        }
        default: {
