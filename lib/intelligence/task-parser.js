...
(function (root, factory) {
  ...
  const ACTIONS = [
    ...
    'SCHEDULE_FOLLOWUP', 'PROVIDE_PAYMENT_INSTRUCTIONS', 'NEGOTIATE_DISCOUNT', 'GRANT_MODULE_ACCESS',
  ];
  ...
  function detectGoals(text, amounts) {
    ...
    // Instructions de paiement manuel / remise (aucune URL de paiement).
    if (/(lien de paiement|payer|paiement|encaisse|mobile money|orange money|mtn money|moov money|\bwave\b)/.test(n)
      || (/(remise|reduction|rabais|negoci)/.test(n) && /(prix|tarif|offre)/.test(n))) {
      const goal = { type: 'PAYMENT', target: null };
      goals.push(goal);
    }
  ...
  function buildPlan(textOrOpts, opts) {
    ...
    for (const goal of goals) {
      switch (goal.type) {
        ...
        case 'PAYMENT': {
          // Champs réels (amount/to/product/discount) inconnus de ce parser
          // purement statistique (nombres/canaux/mots-clés) — laissés `null`
          // ici volontairement : ai-engine/chatOrchestrator.js complète ce
          // payload via une extraction LLM ciblée avant exécution (montant,
          // destinataire, remise demandée), plutôt que d'être deviné par une
          // regex sur un objectif libre.
          plan.push(desc('PROVIDE_PAYMENT_INSTRUCTIONS', channels[0], { tenantId: user.tenantId, engine: user.engine }, user, { priority: 60, immediate: true }));
          break;
        }
        default: {
  ...
  function summarize(text, goals) {
    const labels = { SALES: 'ventes', PROSPECTING: 'prospection', CONTENT: 'contenu', FOLLOWUP: 'suivi/relance', ANALYSIS: 'analyse/rapport', ACCOUNTS: 'comptes & accès', PAYMENT: 'paiement', DEFAULT: 'analyse' };
    return goals.map((g) => {
      const s = [];
