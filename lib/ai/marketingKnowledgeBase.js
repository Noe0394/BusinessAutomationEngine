...
function findObjection(message) {
  ...
  // matche pas mot pour mot. keywordMatches (voir keywordMatch.js) plutôt
  // qu'un simple regex/includes() : un mot court comme "cher" matcherait
  // sinon à tort à l'intérieur de "chercher", "toucher", "approcher"...
  const fallbackMap = [
    { keywords: ['cher', 'prix', 'budget élevé', 'coûte'], phrase: "c'est trop cher" },
    { keywords: ['réfléchir', 'réflexion', 'je verrai', 'plus tard'], phrase: 'je vais réfléchir' },
    { keywords: ['confiance', 'arnaque', 'sûr que', 'garantie', 'garanti'], phrase: "je n'ai pas confiance" },
    { keywords: ['pas le temps', 'trop occupé', 'débordé'], phrase: "je n'ai pas le temps" },
    { keywords: ['déjà un', 'déjà une', 'concurrent', 'fournisseur actuel'], phrase: "j'ai déjà un fournisseur/une solution" },
    { keywords: ['pas les moyens', "pas d'argent", 'fauché', 'petit budget'], phrase: "je n'ai pas les moyens en ce moment" },
  ];
  const match = fallbackMap.find((f) => f.keywords.some((kw) => keywordMatches(text, kw)));
