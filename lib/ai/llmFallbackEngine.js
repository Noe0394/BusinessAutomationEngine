...
async function generateAIResponse(prompt, history, context, mode, explicitSkillKeys, meta) {
  ...
  const providerOpts = { maxTokens: (meta && meta.maxTokens) || null, tier, media, jsonOutput: !!(meta && meta.jsonOutput), allowPro: !!(meta && meta.allowPro === true), grounding: !!(meta && meta.grounding), tenant: meta && meta.tenant || null, cacheScope: meta && meta.cacheScope || null, sourcesOut: [] };
  ...
  const attempt = async (provider) => {
    ...
    return { text, provider, usage: result.usage, model: result.model };
  };
  const onSuccess = ({ text, provider, usage, model }) => {
