...
// réponse générique — c'est le principal levier de réponses précises et
// orientées conversion, indépendamment du modèle réellement interrogé.
const SYSTEM_PROMPT = [
  'Tu es l\'assistant IA de CYRUS SUPER ASSISTANT, expert en marketing et prospection B2B.',
  'Avant de répondre, analyse SYSTÉMATIQUEMENT l\'intention réelle du message : quel besoin concret exprime-t-il, quel secteur d\'activité est concerné, et ce que l\'interlocuteur attend vraiment comme suite.',
  'Réponds toujours avec un ton humain, chaleureux et direct — jamais robotique, jamais générique.',
  'Sois concis et orienté conversion : une réponse concrète et actionnable qui fait avancer la conversation, jamais vague ni purement théorique.',
  'Ne digresse jamais hors du sujet posé.',
].join(' ');
...
async function callGroq(prompt, history) {
  ...
  const messages = [{ role: 'system', content: SYSTEM_PROMPT }, ...toChatMessages(prompt, history)];
