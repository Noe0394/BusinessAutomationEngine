...
async function callGroq(prompt, history, context, mode, skillPromptBlock, opts) {
  ...
  if (opts && opts.maxTokens) body.max_tokens = Math.max(800, Math.min(2048, opts.maxTokens * 3));
  ...
  if (cut(res)) res = await post(Object.assign({}, body, { max_tokens: 2048 }));
...
// compte.
async function callGemini(prompt, history, context, mode, skillPromptBlock) {
...
// autre fournisseur).
async function callDeepSeek(prompt, history, context, mode, skillPromptBlock) {
  ...
  const res = await axios.post(
    ...
    { model: process.env.DEEPSEEK_MODEL || 'deepseek-chat', messages },
    { headers: { Authorization: `Bearer ${apiKey}` }, timeout: REQUEST_TIMEOUT_MS },
  );
  const text = res.data && res.data.choices && res.data.choices[0] && res.data.choices[0].message
    ? res.data.choices[0].message.content
...
function openAiCompatible(name, keyEnv, url, modelEnv, defaultModel) {
  return async function call(prompt, history, context, mode, skillPromptBlock) {
    const apiKey = process.env[keyEnv];
    if (!apiKey) return null;
    const messages = [{ role: 'system', content: buildSystemPrompt(context, mode, skillPromptBlock) }, ...toChatMessages(prompt, history)];
    const res = await axios.post(url, { model: process.env[modelEnv] || defaultModel, messages },
      { headers: { Authorization: `Bearer ${apiKey}` }, timeout: REQUEST_TIMEOUT_MS });
...
const callOpenAI = openAiCompatible('OpenAI', 'OPENAI_API_KEY', 'https://api.openai.com/v1/chat/completions', 'OPENAI_MODEL', 'gpt-4o-mini');
const callMistral = openAiCompatible('Mistral', 'MISTRAL_API_KEY', 'https://api.mistral.ai/v1/chat/completions', 'MISTRAL_MODEL', 'mistral-small-latest');

async function callClaude(prompt, history, context, mode, skillPromptBlock) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return null;
  const msgs = toChatMessages(prompt, history).filter((m) => m.role === 'user' || m.role === 'assistant');
  const res = await axios.post('https://api.anthropic.com/v1/messages',
    { model: process.env.ANTHROPIC_MODEL || 'claude-haiku-4-5-20251001', max_tokens: 1024, system: buildSystemPrompt(context, mode, skillPromptBlock), messages: msgs },
    { headers: { 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' }, timeout: REQUEST_TIMEOUT_MS });
...
async function generateAIResponse(prompt, history, context, mode, explicitSkillKeys, meta) {
  ...
  const providerOpts = { maxTokens: (meta && meta.maxTokens) || null };
    ...
    try {
      ...
      try {
        ...
        require('../../ai-engine/aiUsageLedger').record({
          ...
          purpose: m.purpose || (mode ? String(mode) : 'conversation'),
