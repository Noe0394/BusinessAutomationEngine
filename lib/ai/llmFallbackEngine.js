...
const INTERACTIVE_PROVIDER_TIMEOUT_MS = 5000;
const positiveEnvMs = (name, fallback, max) => {
  const value = Number(process.env[name]);
  return Number.isFinite(value) && value > 0 ? Math.min(Math.floor(value), max) : fallback;
};
const interactiveBudgetFor = (opts) => positiveEnvMs('AI_INTERACTIVE_BUDGET_MS', INTERACTIVE_BUDGET_MS, 30000);
const interactiveProviderTimeoutFor = (opts) => positiveEnvMs('AI_INTERACTIVE_PROVIDER_TIMEOUT_MS', INTERACTIVE_PROVIDER_TIMEOUT_MS, 15000);
