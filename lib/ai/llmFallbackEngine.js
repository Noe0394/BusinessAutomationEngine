...
async function generateAIResponse(prompt, history, context, mode) {
  // Détection de compétence marketing (voir lib/ai/marketingSkills.js) —
  // calculée UNE SEULE FOIS ici, jamais par fournisseur (le même bloc, ou
  // son absence, doit s'appliquer identiquement quel que soit le niveau de
  // la cascade qui finit par répondre).
  const skillKey = marketingSkills.detectSkill(prompt);
  const skillPromptBlock = skillKey ? marketingSkills.buildSkillPromptBlock(skillKey) : '';
