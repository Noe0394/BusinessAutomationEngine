...
const timeoutFor = (opts) => (opts && opts.tier === 'reasoning' ? REASONING_TIMEOUT_MS : REQUEST_TIMEOUT_MS);
...
// compte.
async function callGemini(prompt, history, context, mode, skillPromptBlock, opts) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) return null;
  // Niveau raisonnement : modèle Pro (accessible avec la même clé : gemini-pro-latest / gemini-2.5-pro / gemini-3.1-pro-preview).
  const model = opts && opts.tier === 'reasoning' ? (process.env.GEMINI_REASONING_MODEL || 'gemini-pro-latest') : 'gemini-3.6-flash';
  ...
  const res = await axios.post(
    ...
    { contents, systemInstruction: { parts: [{ text: buildSystemPrompt(context, mode, skillPromptBlock) }] } },
...
async function callOpenRouter(prompt, history, context, mode, skillPromptBlock) {
...
// NIVEAUX D'IA (meta.tier) :
//   'fast' / 'standard' (défaut) : cascade « moins cher d'abord » ci-dessus (conversation courante) ;
//   'reasoning' : décisions CRITIQUES qui demandent de la réflexion (choix d'outil de l'agent, arbitrage d'intention, extraction
//                 structurée fiable, offres) : les modèles les PLUS FORTS d'abord — Claude, OpenAI, puis Groq gpt-oss-120b en effort
//                 « high », puis Gemini Pro, DeepSeek reasoner, Mistral large — avant les modèles rapides/gratuits en secours.
//   Un niveau dont la clé est absente est sauté silencieusement, comme avant.
const byName = (n) => PROVIDERS.find((p) => p.name === n);
function providersForTier(tier) {
  if (tier !== 'reasoning') return PROVIDERS;
  const order = ['claude', 'openai', 'groq', 'gemini', 'deepseek', 'mistral', 'openrouter', 'huggingface', 'pollinations'];
  return order.map(byName).filter(Boolean);
}
// État des connexions IA (noms uniquement, jamais de clé) : qui est configuré, dans quel ordre pour chaque niveau.
const KEY_OF = { groq: 'GROQ_API_KEY', gemini: 'GEMINI_API_KEY', deepseek: 'DEEPSEEK_API_KEY', openrouter: 'OPENROUTER_API_KEY', huggingface: 'HUGGINGFACE_API_KEY', openai: 'OPENAI_API_KEY', mistral: 'MISTRAL_API_KEY', claude: 'ANTHROPIC_API_KEY', pollinations: null };
function getProviderStatus() {
  const configured = (p) => KEY_OF[p.name] === null || !!process.env[KEY_OF[p.name]];
  const list = (tier) => providersForTier(tier).filter(configured).map((p) => p.name);
  return { standard: list('standard'), reasoning: list('reasoning'), missingStrongModels: ['claude', 'openai'].filter((n) => !process.env[KEY_OF[n]]) };
}
...
// l'appelant à ce stade — seul un échec de tous les niveaux (Pollinations compris,
...
// explicitSkillKeys (optionnel) : une clé ou un tableau de clés du registre
// lib/ai/skills/index.js (SKILLS), utilisé par les routes qui SAVENT déjà
// dans quel module du Studio elles se trouvent (voir index.js — génération
// d'affiches, vidéo séquentielle, UGC, montage mobile, faceless shorts) —
// prend le pas sur la détection par mots-clés de lib/ai/marketingSkills.js
// (chat libre, voir Copywriter Studio IA), jamais les deux combinés.
async function generateAIResponse(prompt, history, context, mode, explicitSkillKeys, meta) {
  // Options fournisseur issues du model router (§6) : plafond de tokens de
  // sortie selon la complexité (économie réelle). Ignoré par les fournisseurs
  // qui ne le gèrent pas — seul callGroq (le fournisseur principal) l'applique.
  const tier = (meta && meta.tier) || 'standard';
  const providerOpts = { maxTokens: (meta && meta.maxTokens) || null, tier };
  ...
  // Protection anti-boucle / anti-coût (§29) : si l'appel est rattaché à une
  // tâche (meta.taskId), on refuse au-delà du plafond d'appels IA par tâche.
  // Déterministe, avant tout appel réseau. Lève AI_LOOP_LIMIT (l'appelant gère).
  if (meta && meta.taskId) {
    require('../../ai-engine/loopGuard').countAiCall(meta.taskId);
  }

  const errors = [];
  for (const provider of providersForTier(tier)) {
    try {
      const text = await provider.call(prompt, history, context, mode, skillPromptBlock, providerOpts);
      ...
      // AI Cost Guard (RÈGLE N°2) — point de passage UNIQUE de tous les appels
      // IA : on enregistre l'usage réel (fournisseur, tokens estimés, coût, but,
      // tenant). 100 % déterministe et NON bloquant : jamais d'IA ici, et une
      // erreur d'enregistrement ne casse pas la réponse.
      try {
        const m = meta || {};
