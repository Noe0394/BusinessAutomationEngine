...
async function generateAIResponse(prompt, history, context, mode, explicitSkillKeys, meta) {
  ...
  const providerOpts = { maxTokens: (meta && meta.maxTokens) || null, tier, media, jsonOutput: !!(meta && meta.jsonOutput), allowPro: !!(meta && meta.allowPro === true), grounding: !!(meta && meta.grounding), sourcesOut: [] };
  ...
  const onSuccess = ({ text, provider }) => {
    ...
    return { text, provider: provider.name, sources: providerOpts.grounding ? providerOpts.sourcesOut.slice() : [] };
  };
  ...
  // RÉPONSE SPONTANÉE : pour un texte interactif (standard, sans média ni recherche), si le fournisseur en cours n'a pas répondu au bout de HEDGE_MS,
  ...
  const hedgeMs = (meta && meta.hedge === false) || tier !== 'standard' || media.length || providerOpts.grounding ? 0 : (process.env.AI_HEDGE_MS !== undefined && process.env.AI_HEDGE_MS !== '' ? Math.max(0, Number(process.env.AI_HEDGE_MS) || 0) : 3500);
  if (!hedgeMs) {
