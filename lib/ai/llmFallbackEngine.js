...
async function callClaude(prompt, history, context, mode, skillPromptBlock, opts) {
  ...
  const res = await axios.post('https://api.anthropic.com/v1/messages',
    ...
    { model: 'claude-haiku-4-5-20251001', max_tokens: (opts && opts.tier === 'reasoning') ? 4096 : 1024, system: buildSystemPrompt(context, mode, skillPromptBlock), messages: msgs },
...
async function generateAIResponse(prompt, history, context, mode, explicitSkillKeys, meta) {
  ...
  const hedgeMs = (meta && meta.hedge === false) || media.length || providerOpts.grounding || chain[0].name === 'claude' ? 0 : (tier === 'standard' ? envMs('AI_HEDGE_MS', 3500) : envMs('AI_HEDGE_REASONING_MS', 7000));
  if (!hedgeMs) {
