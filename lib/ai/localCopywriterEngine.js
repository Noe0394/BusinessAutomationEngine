...
// ce pipeline de détection d'intention + composition ci-dessous, qui
// applique EXPLICITEMENT les 3 règles suivantes (demande explicite de
// l'utilisateur — "contraintes d'acier" contre les divagations) :
//   Règle 1 (aucune digression) : composeReply ne retient jamais qu'UN SEUL
//     topic principal (voir `topicKeys.slice(0, 1)` plus bas) — jamais un
//     second sujet seulement partiellement matché ajouté "en plus", qui
//     détournerait la réponse du sujet réellement demandé.
//   Règle 2 (intention précise, concise, empathique) : chaque catégorie
//     d'intention (GREETING/SUPPORT/CLOSING/OBJECTION/QUESTION) a son propre
//     ton dédié (voir OPENERS_BY_PROFILE/EMPATHETIC_OPENERS/CLOSING_OPENERS)
//     et une composition volontairement bornée (pickN limite à 2 sections
//     par sujet) — jamais un pavé exhaustif quand une réponse ciblée suffit.
//   Règle 3 (jamais vague/hors-contexte CYRUS) : composeFallbackReply ne
//     renvoie JAMAIS le menu générique complet par défaut — elle nomme
//     explicitement ce qui n'a pas été compris (cite le message) et demande
//     UNE précision ciblée, toujours dans le périmètre CYRUS (marketing/
//     closing/anti-ban/support produit), jamais une réponse déconnectée.
...
// Demande explicite d'aide/liste des capacités ("aide", "que sais-tu
// faire"...) — SEUL cas où le menu complet (CAPABILITIES_INTRO) est renvoyé
// hors accueil pur, puisqu'il a été explicitement demandé (règle 3 : jamais
// imposé par défaut sur un message non compris, voir composeFallbackReply).
const HELP_RE = /\b(aide|que (sais|peux)[- ]tu faire|tes capacit[ée]s|liste (tes|des) (fonctionnalit[ée]s|commandes)|help|what can you do)\b/i;
...
// Règle 3 (interdiction absolue de répondre vaguement) : plutôt que de
// renvoyer systématiquement le menu générique complet (CAPABILITIES_INTRO,
// 6 puces) quel que soit le message — une forme de réponse "passe-partout"
// qui ne traite jamais VRAIMENT ce qui a été demandé — on nomme
// explicitement ce qui n'a pas été compris (cite le message tel quel,
// tronqué s'il est long) et on pose UNE question de précision ciblée,
// toujours dans le périmètre CYRUS. Reste honnête : le moteur ne devine ni
// n'invente jamais un sujet non matché, il le dit clairement.
function composeFallbackReply(originalText) {
  const trimmed = String(originalText || '').trim();
  const quoted = trimmed.length > 90 ? `${trimmed.slice(0, 90)}…` : trimmed;
  return [
    quoted
      ? `Je n'ai identifié aucun sujet CYRUS précis dans "${quoted}" — plutôt que de deviner, précisez votre besoin pour une réponse exacte.`
      : "Je n'ai pas identifié de sujet précis — précisez votre besoin pour une réponse exacte.",
    '',
    'Par exemple, dans mon périmètre :',
    '• 📣 Une question marketing/closing (ads, structure, objection...) — ex: "structure d\'une pub Facebook"',
    '• 🛠️ Un souci technique CYRUS (connexion WhatsApp, Studio IA, campagnes...) — ex: "mon QR code WhatsApp est rejeté"',
    '',
    'Ou tapez "aide"/"que sais-tu faire" pour voir la liste complète de mes domaines.',
  ].join('\n');
}
...
function composeReply(userMessage, history) {
  ...
  const isHelpRequest = HELP_RE.test(analysisText);
  ...
  if (guide) {
    ...
    // Règle 1 (aucune digression) : UN SEUL topic — le mieux matché — jamais
    // un second sujet ajouté "en prime" qui détournerait la réponse du sujet
    // réellement demandé (voir le commentaire d'en-tête du fichier).
    const chosenTopics = topicKeys.slice(0, 1);
    ...
    body = composeWelcomeReply();
    category = 'GREETING';
  } else {
    body = composeFallbackReply(analysisText);
    category = 'UNKNOWN';
  }
