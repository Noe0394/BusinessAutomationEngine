...
function startPull(getLocal, setLocal, intervalMs) {
  ...
  const run = async (first) => {
    ...
    // Au démarrage : les licences déjà présentes sur le VPS sont poussées vers Cloudflare (une liaison d'appareil
    // faite côté Cloudflare n'est jamais effacée, voir COALESCE côté Worker).
    if (first) await pushLicenses(getLocal());
  };
  run(true).catch((err) => console.error('Synchronisation initiale des licences Cloudflare :', err.message));
  timer = setInterval(() => run(false).catch(() => {}), intervalMs || 15 * 60 * 1000);
