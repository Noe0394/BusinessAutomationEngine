...
async function restore(filePath) {
  ...
  if (!envelope || envelope._cyrusEncrypted !== 1 || envelope.format !== 'cyrus-durable-json-v1') {
    if (isProduction()) throw new Error('DURABLE_FILE_REMOTE_NOT_ENCRYPTED');
    return false;
  }
  const plain = secretVault.decrypt(envelope.payload);
