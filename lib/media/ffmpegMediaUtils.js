...
// Largeur moyenne de glyphe DejaVu Sans Bold pour du texte latin courant —
// approximation empirique (aucune vraie métrique de police disponible côté
// ffmpeg sans libass) utilisée UNIQUEMENT pour réduire préventivement la
// taille de police avant rendu. Constaté en test réel (image composite
// 1024px, sizeFrac 0.08) : un prix un peu long ("5 000 FCFA + livraison")
// dépassait des DEUX côtés de l'image avec la taille de police "brute" —
// ffmpeg ne recadre ni ne réduit jamais automatiquement un texte trop
// large, contrairement à un balisage HTML/CSS classique.
const AVG_GLYPH_WIDTH_RATIO = 0.58;
const MIN_FONTSIZE = 16;

function fitFontSize(text, width, requestedFontSize) {
  const maxWidth = width * 0.92;
  const estimatedWidth = String(text || '').length * requestedFontSize * AVG_GLYPH_WIDTH_RATIO;
  if (estimatedWidth <= maxWidth || estimatedWidth === 0) return requestedFontSize;
  const scaled = Math.floor((maxWidth / estimatedWidth) * requestedFontSize);
  return Math.max(MIN_FONTSIZE, scaled);
}
...
function buildDrawtextFilters(fields, lastLabel, width, height) {
  ...
  fields.forEach((field) => {
    ...
    counter += 1;
    const fontsize = fitFontSize(field.text, width, Math.round(width * field.sizeFrac));
    filters.push(
      `[${label}]drawtext=fontfile='${fontLiteral}':text='${escapeDrawtext(field.text)}':`
      + `fontcolor=white:fontsize=${fontsize}:borderw=4:bordercolor=black@0.85:`
